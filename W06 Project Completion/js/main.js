...
export async function initRecipes(){
  ...
  try {
    ...
    showFavoritesBtn.addEventListener('click', () => renderRecipes(allRecipes.filter(r => favorites.has(r.id))));
    ...
    if(collectionEl) collectionEl.innerHTML = '<p class="error">Unable to load recipes.</p>';
  }
}
...
function renderRecipes(items){
  ...
  toShow.forEach(item => {
    const card = document.createElement('article'); card.className = 'card recipe';
    card.innerHTML = `
...
function applyFilters(){
  ...
  if(q) results = results.filter(i=> (i.title + ' ' + i.desc + ' ' + (i.ingredients||[]).join(' ')).toLowerCase().includes(q));
